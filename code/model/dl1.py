...
# substitution with the mean , see if that makes any difference 
...
# fit network
history = model.fit(train_X, train_y, epochs=25, batch_size=8000, validation_data=(test_X, test_y), verbose=2, shuffle=True)
...
train_X = train_X.reshape((train_X.shape[0], n_hours*n_features))
# invert scaling for forecast
inv_yhat = concatenate((yhat, train_X[:, -(len(list(dataset))-1):]), axis=1)
inv_yhat = scaler.inverse_transform(inv_yhat)
inv_yhat = inv_yhat[:,0]
# invert scaling for actual
train_y = train_y.reshape((len(train_y), 1))
inv_y = concatenate((train_y, train_X[:, -(len(list(dataset))-1):]), axis=1)
inv_y = scaler.inverse_transform(inv_y)
inv_y = inv_y[:,0]
# calculate RMSE
# inv_yhat = np.around(inv_yhat, decimals=4)
...
pyplot.plot(inv_yhat, label='invyhat')
pyplot.legend()
pyplot.savefig("/scratch/sdonthin/4_16.pdf")
pyplot.close()
